...
const App: React.FC = () => {
  ...
  const [currentNodeDescriptions, setCurrentNodeDescriptions] = useState<Record<string, string>>({});
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (mermaidCode) {
        ...
        setCurrentNodeDescriptions(nodeDescriptions || {});
  ...
  const handleVersionSelect = (version: DiagramVersion) => {
    ...
    setCurrentNodeDescriptions(version.nodeDescriptions || {});
  };
        ...
        <div className="flex-1 h-full p-4 flex flex-col gap-4 overflow-hidden relative">
          ...
          <div className={`flex-1 transition-all duration-300 ${showCodePreview ? 'h-2/3' : 'h-full'} relative`}>
            <DiagramRenderer
              ...
              nodeDescriptions={currentNodeDescriptions}
              extraToolbarContent={
                <>
                  <button
